...
if (name %like% "CML"){ name ="CML_uconn_9_5_nup214"}
if (name %like% "fetal"){ name ="normal_fetal_8_15"}
if (name %like% "breast"){ name ="normal_breast_9_6"}
...
if (spork==1){
train.bad=allg[(sample %like% spork) &(!(junction %like% "no_fusion")) & paste(sample)==my.sample & badfj1is1>0 ]
}
if (spork==0){
...
test= all.passed[(emp_p<p.thresh &  junction_cdf.y >lower.jun.cdf/sqrt(numReads.y) & numReads.y>min.reads) | (junction_cdf_lower.y >.5 & numReads.y==1 & productPhat.y>.5)]
